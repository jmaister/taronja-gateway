...
- What a reload can *never* change: `server.host`/`port` (would mean rebinding the listening socket) and the database/session-store connection (`Dependencies` isn't rebuilt) — both require an actual process restart.
