...
**Fingerprint consolidation (`middleware/fingerprint/select.go`, `db.ClientInfo`):** the three signals above (JA4H, TLS JA4, stable) are each still set as their own request header, but `db.ClientInfo` — and so every `Session`/`TrafficMetric` row and `X-User-Data` — stores only two fields: `Fingerprint` and `FingerprintType`. `fingerprint.SelectFingerprint(req)` picks whichever single signal is most reliable and actually present, in priority order `TypeJA4TLS` ("ja4_tls") > `TypeStable` ("stable") > `TypeJA4H` ("ja4h"), returning `("", "")` if none of the three produced anything. `session.NewClientInfo` calls this once and stores the result directly — it no longer reads the three `Get*FromRequest` accessors itself. A direct consumer that wants one specific algorithm rather than "whichever is best" still can, via `fingerprint.GetJA4FromRequest`/`GetStableFingerprintFromRequest`/`GetJA4TLSFromRequest` directly — those didn't go away, `SelectFingerprint` is just one more consumer of them.
...
- Auto-migration on init (schema-as-code via GORM struct tags) for anything AutoMigrate can do by itself — new tables/columns/indexes; see "Data migrations" below for the one thing it can't (rewriting values already on disk) and how this project handles that instead
...
**Any middleware that wraps `http.ResponseWriter` in its own struct must implement `Unwrap() http.ResponseWriter` on it**, or it silently breaks WebSocket proxying for every route that passes through it. `httputil.ReverseProxy`'s upgrade handling calls `http.ResponseController.Hijack()` to take over the raw connection for a `101 Switching Protocols` response; `ResponseController` only follows a wrapper chain via the standard `Unwrap()` convention, and stops — returning `http.ErrNotSupported` — the moment it hits one without it, even if the *real* `http.ResponseWriter` underneath fully supports hijacking. This was a real, previously-unnoticed bug: `logging.go`'s `responseWriter`, `trafficmetric.go`'s `responseWriterWithStats`, `metrics.go`'s `metricsResponseRecorder` (wraps *every* built-in middleware — see `instrumentMiddleware` below), and `ratelimiter.go`'s `statusRecorder` all lacked it, so a WebSocket upgrade proxied through any of logging/traffic_metrics/rate_limiter — the common case, since every sample config enables logging and analytics — failed outright with "can't switch protocols using non-Hijacker ResponseWriter type ...". `compression.go`'s `compressingResponseWriter` already had the fix (and explicitly documents why); the other four didn't, until a session that finally wrote a real WebSocket test against the actual wrapped handler (`gw.handler`, not the unwrapped `gw.Mux` most other gateway tests deliberately use to isolate routing from the global chain — see `gateway/websocket_test.go`) caught it. See the README's "WebSocket Support" section.
