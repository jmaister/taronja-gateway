...
- **Cobra CLI** with five subcommands:
  ...
  - `migrate --config <path> [--force]` — upgrades a config file to the version this build requires, writing a new `-vN`-suffixed file and leaving the original untouched (see "Config file versioning" in the `config/` section below); `run` and `middleware list` both refuse to load a config file older than current and point here
...
- **`tg migrate --config <path> [--force]`** (`main.go`'s `migrateConfigFile`, calling `config.MigrateConfigFile`) is the only supported way to move an outdated config forward. It reads the file, and — if older than current — migrates its *original, pre-`${VAR}`-expansion* bytes (`configMigrations`, one step per version, chained by `migrateConfigToCurrent`) and writes the result to a sibling file (`versionedConfigPath` appends `-v<N>` to the base filename, e.g. `config.yaml` → `config-v2.yaml`), without touching the original. Refuses to overwrite an existing migrated file unless `--force` is passed (it may have been hand-edited since). Already-current input is a no-op (empty `writtenPath`, no error) — callers must check for that rather than treating it as failure.
